...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        transitions = self.lower_bound

        while self.lower_bound <= transitions <= self.upper_bound:
            if current.is_hive or not current:
                return None
            else:
                if current.bees:
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, health=2, ant_contained=None):
        super().__init__(health)
        self.ant_contained = ant_contained
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, gamestate):
        ...
        while current:
            ...
            if my_ant:
                if not my_ant.is_doubled:
                    my_ant.double()
                    my_ant.is_doubled = True
                
                if my_ant.is_container and my_ant.ant_contained:
                    if not my_ant.ant_contained.is_doubled:
                        my_ant.ant_contained.double()
                        my_ant.ant_contained.is_doubled = True
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        target.duration_left = 5
        
        def sticky_syrup(gamestate):
            if not target.duration_left:
                Bee.action(target, gamestate)
            else:
                if gamestate.time % 2 == 0:
                    Bee.action(target, gamestate)

                target.duration_left -= 1

        target.action = sticky_syrup             
